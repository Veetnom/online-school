  ...
  return (
    <Card padding={false} className="overflow-hidden">
      <div className="flex flex-col md:flex-row">
        <div className="w-full border-b border-slate-200 md:w-72 md:border-b-0 md:border-r">
          <div className="border-b border-slate-200 p-4">
            <h2 className="font-semibold text-slate-900">Онлайн-куратор</h2>
          </div>
          {threads.map((thread) => (
            <button
              key={thread.id}
          ...
          <div className="flex min-h-96 flex-1 flex-col">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-4">
              {activeThread.messages.map((msg) => (
                ...
                >
                  <div className="max-w-[80%] rounded-xl bg-slate-100 px-4 py-3">
                    <p className="text-sm text-slate-800">{msg.text}</p>
                    <p className="mt-1 text-right text-xs text-slate-400">{msg.time}</p>
                  </div>
                </div>
              ))}
            </div>
